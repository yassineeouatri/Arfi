...
class product_order_test_wizard(models.Model):
    ...
    test_ids = fields.Many2many('product.mesure.test','rel_order_test',
                                                  'order_id','test_id','test')
    
    def action_execute(self):
        for obj in self.test_ids:
            self.env['product.order.test'].create({'order_id' : self.order_id.id,
                                                       'test_id' : obj.id})
...
class product_order_test_delete_wizard(models.Model):
    ...
    def action_delete(self):
        obj=self.env['product.order.test'].search([('id', '=', self.order_test_id.id)])
        if obj:
            obj.unlink()
        return {
