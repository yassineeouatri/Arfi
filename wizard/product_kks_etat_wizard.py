...
class product_kks_etat_wizard(models.Model):
    ...
    type_pdr = fields.Selection(
        [
            ("type1", "Pièces à sortir du magasin"),
            ("type2", "Liste générale PDR pour l'arrêt"),
        ],
        "Type PDR",
    )
    type_outillage = fields.Selection(
        [("type1", "Essai sur site"), ("type2", "Révision")], "Type outillage"
    )
    type_travaux = fields.Selection(
        [
            ("1", "Tout"),
            ("2", "Révision"),
            ("3", "Révision Chaudière"),
            ("4", "Révision Machine"),
            ("5", "Révision Vanne"),
            ("6", "Révision Vanne Chaudière"),
            ("7", "Révision Vanne Machine"),
            ("8", "Révision Soupapes"),
            ("9", "Révision Soupapes Chaudière"),
            ("10", "Révision Soupapes Machine"),
            ("11", "Changement Vanne"),
            ("12", "Changement Vanne Chaudière"),
            ("13", "Changement Vanne Machine"),
            ("14", "Changement PE"),
            ("15", "Tarage"),
            ("16", "Tarage sur site"),
    ...
    type_contrat = fields.Selection(
        [("Contrat", "Contrat"), ("BC", "BC")], "Contrat/BC"
    )
    ...
    def generer_pdr_report_excel2(self, data):
        ...
        feuille.write("F" + str(x), "Sécurité", style_title)
        feuille.write("G" + str(x), "Recommandé", style_title)
        feuille.write("H" + str(x), "Quantité à sortir", style_title)
        feuille.write("I" + str(x), "Quantité à commander", style_title)
        feuille.write("J" + str(x), "Quantité commandée", style_title)
        feuille.write("K" + str(x), "Designation", style_title)
        feuille.write("L" + str(x), "item", style_title)
        feuille.write("M" + str(x), "KKS", style_title)
        feuille.write("N" + str(x), "Refference", style_title)
        feuille.write("O" + str(x), "Marque", style_title)

        records = self.env["product.kks.pdr.full.report"].search(
    ...
    def generer_pdr_report_excel1(self, data):
        ...
        feuille.set_tab_color("yellow")
        feuille.set_column("A:A", 16.71)
        feuille.set_column("B:D", 10)
        feuille.set_column("E:E", 76)
        feuille.set_column("F:F", 6)
        feuille.set_column("G:G", 15)
        feuille.set_column("H:H", 53)
        feuille.set_column("I:I", 22)
        feuille.merge_range("A1:I1", "Liste des PDR à sortir du magasin", style_titre)
        record = self.env["product.kks.arret"].search(
            [("arret_id", "=", arret_id)], limit=1
        )
        titre = (
        ...
        feuille.write("D" + str(x), "Unité", style_title)
        feuille.write("E" + str(x), "Designation", style_title)
        feuille.write("F" + str(x), "item", style_title)
        feuille.write("G" + str(x), "KKS", style_title)
        feuille.write("H" + str(x), "Refference", style_title)
        feuille.write("I" + str(x), "Marque", style_title)

        records = self.env["product.kks.pdr.sortir.report"].search(
            [("customer_id", "=", customer_id), ("arret_id", "=", arret_id)]
        )
        x = x + 1
        for record in records:
            if record.row == record.nbr:
                ...
                feuille.write("C" + str(x), record.stock, style)
                feuille.write("D" + str(x), record.unite_magasin, style)
                feuille.write("E" + str(x), record.designation, style_)
                feuille.write("F" + str(x), record.item, style)
                feuille.write("G" + str(x), record.kks, style)
                feuille.write("H" + str(x), record.reference, style_)
                feuille.write("I" + str(x), record.maker_id.name, style)
            else:
                feuille.write("A" + str(x), record.magasin_id.code, style1)
                feuille.write("B" + str(x), "", style1)
                feuille.write("C" + str(x), "", style1)
                feuille.write("D" + str(x), "", style1)
                feuille.write("E" + str(x), record.designation, style1_)
                feuille.write("F" + str(x), record.item, style1)
                feuille.write("G" + str(x), record.kks, style1)
                feuille.write("H" + str(x), record.reference, style1_)
                feuille.write("I" + str(x), record.maker_id.name, style1)
            x = x + 1
    ...
    def _print_pdr_report(self, data):
        ...
        return self.env["report"].get_action(
            self, "arfi.action_report_productkkspdr", data=data
        )

    def _print_pdr_report2(self, data):
        data["form"].update(self.read(["customer_id", "arret_id"])[0])
        return self.env["report"].get_action(
            self, "arfi.action_report_productkkspdrsortir", data=data
        )
    ...
    def _print_echafaudage_report(self, data):
        data["form"].update(self.read(["customer_id", "arret_id"])[0])
        return self.env["report"].get_action(
            self, "arfi.action_report_productkksechafaudagearret", data=data
        )
    ...
    def _print_outillage_report(self, data):
        data["form"].update(self.read(["customer_id", "arret_id"])[0])
        return self.env["report"].get_action(
            self, "arfi.action_report_productkksoutillage", data=data
        )

    def _print_outillage_report2(self, data):
        data["form"].update(self.read(["customer_id", "arret_id"])[0])
        return self.env["report"].get_action(
            self, "arfi.action_report_productkksoutillagetarage", data=data
        )
    ...
    def print_facture_report(self):
        data = {}
        data["form"] = self.read(
            ["customer_id", "arret_id", "type_travaux", "type_contrat"]
        )[0]
        if data["form"]["type_travaux"] == "17":
            return self._print_facture_report_(data)
        elif data["form"]["type_travaux"] in (
            "1",
            "2",
            "3",
            "4",
            ...
            if data["form"]["type_travaux"] in (
                ...
                nature_id = (
                    self.env["product.nature"]
                    .search([("name", "=", "Révision")], limit=1)
                    .id
                )
                if nature_id:
                ...
                if data["form"]["type_travaux"] == "3":
                    ...
                    type_implantation_id = (
                        self.env["product.type.implantation"]
                        .search([("name", "=", "Chaudière")], limit=1)
                        .id
                    )
                    if type_implantation_id:
                        requete = (
                            requete
                            + " and type_implantation_id="
                            + str(type_implantation_id)
                        )
                if data["form"]["type_travaux"] == "4":
                    type_extraction = "Révision machine"
                    type_implantation_id = (
                        self.env["product.type.implantation"]
                        .search([("name", "=", "Machine")], limit=1)
                        .id
                    )
                    if type_implantation_id:
                        requete = (
                ...
                if data["form"]["type_travaux"] in ("5", "6", "7"):
                    type_appareil_id = (
                        ...
                        .search([("name", "=", "VANN"), ("type", "=", "type")], limit=1)
                        .id
                    )
                    ss_type_appareil_ids = self.env["product.category"].search(
                        [("parent_id", "=", type_appareil_id), ("type", "=", "ss_type")]
                    )
                    obj_ids = ""
                ...
                if data["form"]["type_travaux"] in ("8", "9", "10"):
                    type_appareil_id = (
                        self.env["product.category"]
                        .search([("name", "=", "SOUP"), ("type", "=", "type")], limit=1)
                        .id
                    )
                    ss_type_appareil_ids = self.env["product.category"].search(
                        [("parent_id", "=", type_appareil_id), ("type", "=", "ss_type")]
            ...
            if data["form"]["type_travaux"] in ("11", "12", "13"):
                self._cr.execute(
                    "select id from product_nature where name like 'Changement siège' or name like 'Sou%' or name like 'Changement siège'"
                )
                type_appareil_id = (
                    self.env["product.category"]
                    .search([("name", "=", "VANN"), ("type", "=", "type")], limit=1)
                ...
                for obj in ss_type_appareil_ids:
                    obj_ids += "," + str(obj.id)
                requete = (
                    requete + " and ss_type_appareil_id in (" + str(obj_ids[1:]) + ")"
                )
                ids = ""
                nature_ids = self.env["product.nature"].search(
                    [
                        (
                            "name",
                            "in",
            ...
            if data["form"]["type_travaux"] == "14":
                ...
                nature_id = (
                    self.env["product.nature"]
                    .search([("name", "=", "Changement PE")], limit=1)
                    .id
                )
                if nature_id:
            ...
            self._cr.execute(
                "insert into product_kks_facture(name,date_facture,arret_id,customer_id,type_extraction,montant_text,type_facture)\
                                values('Facture',date(now()),"
                + str(data["form"]["arret_id"][0])
                + ","
                + str(data["form"]["customer_id"][0])
                + ",'"
                + type_extraction
                + "','"
                + self.trad(134552.30, "dirham", "centime")
                + "','DEM')"
            )
            facture_id = self.env["product.kks.facture"].search(
                [("name", "=", "Facture")], limit=1
            )
            self.write({"facture_id": facture_id.id})
            self._cr.execute(
                "select item,reference,maker_id,montant,kks_id,unite_id,travaux_id,nature_id,ligne from product_kks_facture_arret\
                                where customer_id="
                + str(data["form"]["customer_id"][0])
                + "\
                                and arret_id="
            ...
            self._cr.execute(
                "select distinct unite_id from product_kks_facture_arret\
                                where customer_id="
                + str(data["form"]["customer_id"][0])
                + "\
                                and arret_id="
            ...
            for res in self.env.cr.fetchall():
                if res[0]:
                    unite_id = res[0]
                    self._cr.execute(
                        "update product_kks_facture set unite_id=" + str(unite_id)
                    )
            montant_ttc = 0
            self._cr.execute(
                "select montant_ttc from product_kks_facture where name='Facture'"
            )
            for res in self.env.cr.fetchall():
                if res[0]:
                    montant_ttc = res[0]
            self._cr.execute(
                "update product_kks_facture set montant_text='"
                + self.trad(montant_ttc, "dirham", "centime")
                + "'"
            )
            return {
                ...
                "views": [
                    (self.env.ref("arfi.view_facture_changement_form").id, "form")
                ],
            }
        else:
            raise UserError(
                _("Merci de remplir le champs (Type Travaux et Type Contrat)")
            )

    def _print_facture_report_(self, data):
        data["form"].update(
            self.read(["customer_id", "arret_id", "type_travaux", "type_contrat"])[0]
        )
        return self.env["report"].get_action(
            self, "arfi.action_report_productkksfactureechafaudagearret", data=data
        )
    ...
    def print_appelCommande_report_(self, data):
        ...
        style_titre3 = workbook.add_format(
            {
                "text_wrap": True,
                "bold": 1,
                "align": "center",
        ...
        feuille.merge_range(
            "F" + str(x) + ":G" + str(x), "Désignation Pièce", style_title
        )
        ...
        records = self.env["product.kks.appel.commande.report"].search(
            [
        ...
        for record in records:
            ...
            feuille.merge_range(
                "F" + str(x) + ":G" + str(x), record.designation, style1
            )
            if record.magasin_id.unite_magasin:
                feuille.write(
                    "H" + str(x), record.magasin_id.unite_magasin.upper(), style1
                )
            else:
                feuille.write("H" + str(x), "", style1)
        ...
        feuille.merge_range(
            "F" + str(x + 1) + ":G" + str(x + 1), "Total Hors Taxe", style_titre3
        )
        feuille.write_formula(
            "K" + str(x + 1), "=SUM(K22:K" + str(x - 1) + ")", style_titre3
        )
    ...
    def _print_poids_report(self, data):
        data["form"].update(self.read(["customer_id", "arret_id"])[0])
        return self.env["report"].get_action(
            self, "arfi.action_report_productkkspoids", data=data
        )
