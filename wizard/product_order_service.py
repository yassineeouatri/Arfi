...
class product_order_service_wizard(models.Model):
    ...
    def action_execute(self):
        for obj in self.service_ids:
            self.env["product.order.service"].create(
                {"order_id": self.order_id.id, "service_id": obj.id}
            )
...
class product_order_service_delete_wizard(models.Model):
    ...
    def action_delete(self):
        obj = self.env["product.order.service"].search(
            [("id", "=", self.order_service_id.id)]
        )
        if obj:
            obj.unlink()
        return {
