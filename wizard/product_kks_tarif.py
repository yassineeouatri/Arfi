...
class product_kks_tarif_wizard(models.Model):
    ...
    nature_ids = fields.Many2many('product.nature','rel_kks_nature',
                                                  'kks_id','nature_id','Natures')
    ...
    def action_execute(self):
        for obj in self.nature_ids:
            ...
            if self.kks_id.appareil_id:
                contract_obj = self.env['product.appareil.price'].search([('appareil_id','=',self.kks_id.appareil_id.id),
                                                              ('nature_id','=',obj.id)],limit = 1)
                if contract_obj :
                    montant2 = contract_obj.purchase_price
                    montant = contract_obj.sale_price
            self.env['product.kks.tarif'].create({'kks_id' : self.kks_id.id,
                                                       'nature_id' : obj.id,
                                                       'fact' : 'Contrat',
                                                        'montant' : montant,
                                                        'montant2' : montant2
