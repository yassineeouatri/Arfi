            ...
            if extension=='csv':
                ...
                for line in lines[:-1]:
                    if rownum == 0:
                        ...
                        magasin_id=self.env['product.magasin'].search([('code','=',magasin)],limit=1).id
                        if magasin_id and info_id:
                            rslt=obj_stock.search([('magasin_id','=',magasin_id),('info_id','=',info_id)],limit=1)
                            if rslt :
                ...
                while curr_row < num_rows:
                    ...
                    if curr_row==0:
                        ...
                        if magasin_id and info_id:
                            rslt=obj_stock.search([('magasin_id','=',magasin_id),('info_id','=',info_id)],limit=1)
                            if rslt :
                                self._cr.execute("update product_kks_stock set value='"+str(stock)+"' \
                                            where magasin_id="+str(magasin_id)+"\
                                            and info_id="+str(info_id))
                                #rslt.write({'value' : stock})
                            else : 
                                self._cr.execute("insert into product_kks_stock(magasin_id,info_id,value)\
                                         values ('"+str(magasin_id)+"',"+str(info_id)+","+str(stock)+")")
                                #obj_stock.create({'magasin_id' : magasin_id,'info_id' : info_id, 'value' : stock})
                ...
                for row in first_sheet.iter_rows():
                    ...
                    if j==1 :
                        ...
                        if magasin_id and info_id:
                            ...
                            first_sheet['A'+str(j)].fill = PatternFill(start_color='00FF00', end_color='00FF00', fill_type = "solid")
                            first_sheet['B'+str(j)].fill = PatternFill(start_color='00FF00', end_color='00FF00', fill_type = "solid")
                            first_sheet['C'+str(j)].fill = PatternFill(start_color='00FF00', end_color='00FF00', fill_type = "solid")
                        else :
                            first_sheet['C'+str(j)] = "Non"
                            first_sheet['A'+str(j)].fill = PatternFill(start_color='FF0000', end_color='FF0000', fill_type = "solid")
                            first_sheet['B'+str(j)].fill = PatternFill(start_color='FF0000', end_color='FF0000', fill_type = "solid")
                            first_sheet['C'+str(j)].fill = PatternFill(start_color='FF0000', end_color='FF0000', fill_type = "solid")
                workbook.save(directory+data.file_name)
            else :
...
class product_magasin_stock_wizard(models.Model):
    ...
    info_ids = fields.Many2many('product.info','rel_magasin_info',
                                                  'magasin_id','info_id','Infos')
    
    def action_execute(self):
        for obj in self.info_ids:
            self.env['product.kks.stock'].create({'magasin_id' : self.magasin_id.id,
                                                       'info_id' : obj.id,
                                                       })
