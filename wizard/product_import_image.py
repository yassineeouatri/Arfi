...
class product_wizard(models.Model):
    ...
    def import_operation(self):
        ...
        i=0
        _logger.info(i)
        self._cr.execute("""select code_order,code_operation,code_piece,poids_matiere,coef_matiere,allotted_time,estimated_cost,
        ...
        for res in self.env.cr.fetchall():
            ...
            self._cr.commit()
            self._cr.execute("update operation set download='t' where id="+str(operation_id))
            self._cr.commit()
        return True
        ...
        for data in self.browse():
            ...
            for line in lines[:-1]:
                if rownum == 0:
                    ...
                    code_order= code_operation=code_piece =intervenant=mat_sal=''
                    poids_matiere= coef_matiere= allotted_time= estimated_cost= spent_time= real_cost= price=0
                    data=line.split(";")
                    i=0
                    _logger.info(i)
                    while i<len(header):
    ...
    def import_image_appareil__(self):
        results = self.env['product.image.directory'].search([('type','=','appareil')])
        for result in results:
            directory_appareil=result.name
        self._cr.execute("select no_app from product_template where param1 is not null order by no_app")
        for res in self.env.cr.fetchall():
            image_path=directory_appareil+str(res[0])+'.png'
            _logger.info(image_path)
            if os.path.exists(image_path):
                template_ids=self.env['product.template'].search([('no_app','=',res[0])])
                if template_ids:
                    template_ids.write( {'default_code':1,'image': self.download_image(image_path)})    
        return {'type': 'ir.actions.act_window_close'}
    def import_image_piece(self):
        results = self.env['product.image.directory'].search([('type','=','piece')])
        for result in results:
            directory_piece=result.name
        self._cr.execute("select code_piece from product_piece where param1 is not null order by code_piece")
        for res in self.env.cr.fetchall():
            image_path=directory_piece+str(res[0])+'.png'
            _logger.info(image_path)
            if os.path.exists(image_path):
                template_ids=self.env['product.piece'].search([('code_piece','=',res[0])])
                if template_ids:
                    template_ids.write( {'default_code':1,'image': self.download_image(image_path)})    
        return {'type': 'ir.actions.act_window_close'}
...
class product_image_directory(models.Model):
    ...
    type = fields.Selection([('appareil','Appareil'),('piece','Pièce'),('reporting','Reporting')],string='Dossier')
