...
class product_order(models.Model):
    ...
    def _needaction_count(self, domain=None):
        cr = self.env.cr
        cr.execute("""select max(cast(no_arfi as numeric))+1 from product_order 
                    where no_arfi ~ '^[0-9\.]+$'
    ...
    def _compute_piece_count(self):
        read_group_res = self.env['product.order.operation'].read_group([('order_id', 'in', self.ids)], ['order_id'], ['order_id'])
        group_data = dict((data['order_id'][0], data['order_id_count']) for data in read_group_res)
        for obj in self:
    ...
    def _amount_all(self):
        ...
        for order in self:
            ...
            allotted_time_operation = estimated_cost_operation = spent_time_operation=real_cost_operation=price_operation=0.0
            allotted_time_piece = estimated_cost_piece = spent_time_piece=real_cost_piece=price_piece=0.0
    ...
    name = fields.Char("Nom", required=True, copy=False, readonly=True, states={'draft': [('readonly', False)]}, index=True, default=lambda self: _('/'))
    ...
    place_id = fields.Many2one('res.partner.place', 'Lieu',
                                domain="[('partner_id','=',customer_id)]",
                                change_default=True,help="")
    maker_id = fields.Many2one('product.template.maker','Fabriquant')
    type_appareil_id = fields.Many2one('product.category','Type Appareil' ,domain=[('type','=','type')])
    ss_type_appareil_id = fields.Many2one('product.category','Sous Type Appareil', required=False, 
    ...
    essai_org = fields.Many2one('product.essai.organisme','Organisme',domain=[('type','=','organisme')])
    essai_nom = fields.Many2one('product.essai.organisme','Nom',domain=[('type','=','person')])
    essai_choix = fields.Integer('Essai Choix')
    essai_file = fields.Binary('Certificat')
    essai_file_name = fields.Char('Fichiers')
    essai_ = fields.Selection([('soupape','Soupape'),('vanne','Vanne'),('vide','Vide')],'Information tarage & épreuve')
    ...
    service_ids = fields.One2many('product.order.service','order_id','Services',copy=True)
    test_ids = fields.One2many('product.order.test','order_id','Test et contrôles',copy=True)
    operation_appareil_ids = fields.One2many('product.order.operation','order_id','Opérations'
                                    ,domain=[('piece_id','=',None)],copy=True)
    operation_piece_ids = fields.One2many('product.order.operation','order_id','Opérations',
                                    domain=[('piece_id','!=',None)],copy=True)
    operation_piece_image_ids = fields.One2many('product.order.piece.image','order_id','Images des pièces')
    operation_appareil_image_ids = fields.One2many('product.order.appareil.image','order_id','Images des pièces')
    state = fields.Selection([('new', 'Nouvelle'),
                          ('confirmed', 'Confirmée'),
                          ('cancelled', 'Annulée')],default='new')
    nbr = fields.Integer('Nbr',default=1)
    
    allotted_time_total = fields.Float(compute='_amount_all',string='Temps Alloué',method=True)
    estimated_cost_total = fields.Float(compute='_amount_all',string='Coût Estimatif',method=True)
    spent_time_total = fields.Float(compute='_amount_all',string='Temps Passé',method=True)
    real_cost_total = fields.Float(compute='_amount_all',string='Coût réel',method=True)
    price_total = fields.Float(compute='_amount_all',string='Prix',method=True)
    allotted_time_operation_total = fields.Float(compute='_amount_all',string='Temps Alloué',method=True)
    estimated_cost_operation_total = fields.Float(compute='_amount_all',string='Coût Estimatif',method=True)
    spent_time_operation_total = fields.Float(compute='_amount_all',string='Temps Passé',method=True)
    real_cost_operation_total = fields.Float(compute='_amount_all',string='Coût réel',method=True)
    price_operation_total = fields.Float(compute='_amount_all',string='Prix',method=True)
    allotted_time_piece_total = fields.Float(compute='_amount_all',string='Temps Alloué',method=True)
    estimated_cost_piece_total = fields.Float(compute='_amount_all',string='Coût Estimatif',method=True)
    spent_time_piece_total = fields.Float(compute='_amount_all',string='Temps Passé',method=True)
    real_cost_piece_total = fields.Float(compute='_amount_all',string='Coût réel',method=True)
    ...
    operation_count = fields.Integer(compute="_compute_piece_count",type='integer', string="# of operation",method=True) 
    ...
    def name_get(self):
        ...
        resuhh = []
        for record in self:
            resuhh.append((record.id,str(record.no_order)+' - '+str(record.kks_id.name)))
        return resuhh
    @api.model
    ...
    def copy(self, default=None):
        ...
        new_id=id.id
        _logger.info("update product_order_operation set real_cost=0,price=0,spent_time=0,intervenant='' where order_id="+str(new_id))
        self._cr.execute("update product_order_operation set real_cost=0,price=0,spent_time=0,intervenant='' where order_id="+str(new_id))
    ...
    def _print_product_order_operation(self, data):
        ...
        return self.env['report'].get_action(self, 'arfi.action_report_productorderoperation', data=data)
    ...
    def _print_product_order_operation_empty(self, data):
        data['form'].update(self.read(['id'])[0])
        return self.env['report'].get_action(self, 'arfi.action_report_productorderoperationempty', data=data)
    ...
    def _print_product_order_devis(self, data):
        data['form'].update(self.read(['id'])[0])
        return self.env['report'].get_action(self, 'arfi.action_report_productorderdevis', data=data)
    @api.multi
    ...
    def _print_product_order_rapport(self, data):
        data['form'].update(self.read(['id'])[0])
        return self.env['report'].get_action(self, 'arfi.action_report_productorderrapport', data=data)
    @api.multi
    def action_appareil(self):
        ...
        if group_hr_atelier in user.groups_id:
            if self.appareil_id:
                appareils=self.env['product.template'].search([('id', '=', self.appareil_id.id)])                
                action = {
                                    ...
                                    'views': [(self.env.ref('arfi.product_order_appareil_create_view_form').id, 'form')],
                                   
                                }
                
                return action
            raise ValidationError(_("Attention! Cette commande n'est affectée à aucun appareil."))
        else :
            if self.appareil_id:
                appareils=self.env['product.template'].search([('id', '=', self.appareil_id.id)])
                if appareils:
                    appareils.write({'order_id' :self.id})
                pieces=self.env['product.piece'].search([('appareil_id', '=', self.appareil_id.id)])
                if pieces:
                    pieces.write({'order_id' :self.id})
                operation=self.env['product.order.operation'].search([('appareil_id', '=', self.appareil_id.id)])
                if operation:
    ...
    def action_realisation(self):        
        return {
            ...
            'view_type': 'form',
            'res_id': self.id,
            'views': [(self.env.ref('arfi.product_order_operation_create_view_form').id, 'form')],
            #'target': 'new',
    ...
    def action_expertise(self):   
        return {
            ...
            'view_type': 'form',
            'res_id': self.id,
            'views': [(self.env.ref('arfi.product_order_expertise_create_view_form').id, 'form')],
            'target': 'new',
            'flags': {'form': {'action_buttons': True}}
             }
    ...
    def action_certificate(self):
        ...
        return {
            ...
            'view_type': 'form',
            'res_id': self.id,
            'views': [(self.env.ref('arfi.product_order_certificate_create_view_form').id, 'form')],
            'target': 'new',
            'flags': {'form': {'action_buttons': True}}
             }
    def action_select_service(self):
        ...
        for obj in self.service_ids:
            self._cr.execute("update product_mesure_service set variant='f' where id="+str(obj.service_id.id))
            self._cr.commit()
        return  {
                'type': 'ir.actions.act_window',
                ...
                'view_mode': 'form',
                'view_type': 'form',
                'views': [(self.env.ref('arfi.view_product_order_service_wizard_form').id, 'form')],
    ...
    def action_select_test(self):
        ...
        for obj in self.test_ids:
            self._cr.execute("update product_mesure_test set variant='f' where id="+str(obj.test_id.id))
            self._cr.commit()
        return  {
                'type': 'ir.actions.act_window',
                ...
                'view_mode': 'form',
                'view_type': 'form',
                'views': [(self.env.ref('arfi.view_product_order_test_wizard_form').id, 'form')],
                'context' : {'default_order_id' : self.id},
                'target': 'new',
                 }
    ...
    def _onchange_appareil(self):
        ...
        if self.appareil_id and self._origin:
            self._cr.execute("update product_order_operation set appareil_id="+str(self.appareil_id.id)+"  where order_id="+str(self._origin.id))
            self._cr.commit()
    ...
    def _onchange_kks(self):
        if self.kks_id:
            if self.kks_id.reference!=self.reference_appareil:
                raise ValidationError(_("Attention! La référence du KKS ne correspond pas à la référence de la commande."))    
    ...
    def create(self, vals):
        if vals.get('name', _('New')) == _('New'):
            if 'company_id' in vals:
                vals['name'] = self.env['ir.sequence'].with_context(force_company=vals['company_id']).next_by_code('product.order') or _('/')
            else:
                vals['name'] = self.env['ir.sequence'].next_by_code('product.order') or _('/')
...
class product_order_appareil_image(models.Model):
    ...
    _sql_constraints = [
            ('name_uniq', 'unique (order_id,appareil_id)', "Attention! Enregistrement unique"),
    ]
class product_order_service(models.Model):
    ...
    def action_delete(self):        
        res=self.env['ir.actions.act_window'].for_xml_id('arfi','open_product_order_service_delete_wizard')
...
class product_order_test(models.Model):
    ...
    def action_delete(self):        
        res=self.env['ir.actions.act_window'].for_xml_id('arfi','open_product_order_test_delete_wizard')
...
class product_order_operation(models.Model):
    ...
    appareil_id = fields.Many2one('product.template','Appareil',domain=[('type','=','appareil')],required=False)
    piece_id = fields.Many2one('product.piece', 'Pièce' )
    order_operation = fields.Float(related='piece_id.order_',string="REP", store=True ,readonly=True)
    code_order = fields.Char('Code Commande')
    ...
    real_cost = fields.Float(compute='_compute_real_cost', inverse='_inverse_real_cost',string='Coût réel', readonly=False, store=True)
    ...
    def open_piece(self):
        if self.piece_id:
            self._cr.execute("select id from product_order_piece_image where order_id="+str(self.order_id.id)+" and piece_id="+str(self.piece_id.id))
            ...
            if res_id:
                return {
                    ...
                    'view_type': 'form',
                    'res_id': res_id,
                    'views': [(self.env.ref('arfi.product_order_piece_image_form_view').id, 'form')],
                    'target': 'new',
                    'flags': {'form': {'action_buttons': True}}}
    def open_appareil(self):
        if self.appareil_id:
            self._cr.execute("select id from product_order_appareil_image where order_id="+str(self.order_id.id)+" and appareil_id="+str(self.appareil_id.id))
            for res in self.env.cr.fetchall():
                res_id=res[0]
            if res_id:
                return {
                    ...
                    'view_type': 'form',
                    'res_id': res_id,
                    'views': [(self.env.ref('arfi.product_order_appareil_image_form_view').id, 'form')],
                    'target': 'new',
                    'flags': {'form': {'action_buttons': True}}}
    @api.model
    def create(self, vals):
    ...
    _sql_constraints = [
            ('name_uniq', 'unique (order_id,operation_id)', "Attention! Enregistrement unique"),
    ]
class operation(models.Model):
    ...
    def _compute_amount(self):
        ...
        for line in self:
            line.update({
                'marge': line.price-line.real_cost
            })
...
class commande_update(models.Model):
    ...
    def update_commande(self):
        rownum=0
        self._cr.execute("""select  com.code_order,no_order,date_order,no_arfi,par.id,pl.id,mak.id,cat.id,
        ...
        self._cr.execute("""update product_order a set customer_id=t.id
                            from(
        ...
        self._cr.commit()
        self._cr.execute("""update product_order ord set kks_id=t.id
        ...
        self._cr.commit()
        self._cr.execute("""update product_order ord set appareil_id=t.id
        ...
        self._cr.commit()
        self._cr.execute("""insert into product_order_operation(code_order,code_operation,code_piece,poids_matiere,coef_matiere,allotted_time,estimated_cost,spent_time,
        ...
        self._cr.commit()
        self._cr.execute("""update product_order_operation a set appareil_id=b.appareil_id
                            from product_order b where b.code_order=a.code_order """)
        self._cr.commit()
        self._cr.execute("""update product_order_operation a set piece_id=b.id
                            from product_piece b where b.code_piece=a.code_piece """)
        self._cr.commit()
        self._cr.execute(""" update product_order_operation a set order_id=b.id
                            from product_order b where b.code_order=a.code_order""")
        self._cr.commit()
        self._cr.execute(""" update product_order_operation a set operation_id=b.id
