...
class product_kks(models.Model):
    ...
    ss_type_appareil_id = fields.Many2one(
        "product.category", "Sous Type Appareil", domain=[("type", "=", "ss_type")]
    )
    ...
    echafaudage_ids = fields.One2many(
        "product.kks.echafaudage", "kks_id", "Echafaudages", copy=False
    )
    ...
    def download_file(self, file_id):
        ...
        self._cr.execute(
            "select filename,file_extension,type_file,file from muk_dms_file a\
        ...
        for res in self.env.cr.fetchall():
            ...
            data = res[3]
        destination = (
            "C:\\Users\\Yassine\\workspace\\arfi_2010\\addons\\web\\static\\reporting\\"
            + filename
        )
    ...
    def action_select_pps(self):
        ...
        return {
            ...
            "view_type": "form",
            "views": [
                (self.env.ref("arfi.view_product_kks_pps_select_form").id, "form")
            ],
            "context": {
                "default_kks_id": self.id,
    ...
    def action_select_tarif(self):
        ...
        return {
            ...
            "view_type": "form",
            "views": [
                (self.env.ref("arfi.view_product_kks_tarif_wizard_form").id, "form")
            ],
    ...
    def action_select_piece(self):
        ...
        self._cr.commit()
        self._cr.execute(
            "update product_piece set variant='t' where appareil_id="
        ...
        return {
            ...
            "view_type": "form",
            "views": [
                (self.env.ref("arfi.view_product_kks_piece_wizard_form").id, "form")
            ],
            "context": {
                "default_kks_id": self.id,
    ...
    def _print_product_kks_echafaudage(self, data):
        ...
        return self.env["report"].get_action(
            self, "arfi.action_report_productkksechafaudage", data=data
        )
    ...
    def action_observation(self):
        ...
        for record in self.piece_ids:
            ...
            info_id = (
                self.env["product.info"].search([("name", "=", "Stock")], limit=1).id
            )
            if record.magasin_id and info_id:
                stock = (
                    self.env["product.kks.stock"]
                    .search(
                        [
                            ("info_id", "=", info_id),
                            ("magasin_id", "=", record.magasin_id.id),
                        ]
                    )
                    .value
                )
            if record.piece_prevoir == True:
                text1 += (
                    '<span width="100%" style="font-size:13px;">\
                        <span style="width:7%;display: inline-block;"> -Rep : '
                    + no_piece.replace("'", "''")
                    + ' </span>\
                        <span style="width:40%;display: inline-block;"> '
                    + piece.replace("'", "''")
                    + ' </span>\
                        <span style="width:10%;display: inline-block;"> Réf : '
                    + ref_fab.replace("'", "''")
                    + ' </span>\
                        <span style="width:15%;display: inline-block;"> code : '
                    + magasin.replace("'", "''")
                    + ' </span>\
                        <span style="width:10%;display: inline-block;"> Stock : '
                    + str(stock)
                    + ' </span>\
            ...
            if record.piece_changee == True:
                text2 += (
                    '<span width="100%" style="font-size:13px;">\
                        <span style="width:15%;display: inline-block;"> -Rep : '
                    + no_piece.replace("'", "''")
                    + ' </span>\
                        <span style="width:40%;display: inline-block;"> '
                    + piece.replace("'", "''")
                    + ' </span>\
                        <span style="width:15%;display: inline-block;"> Réf : '
                    + ref_fab.replace("'", "''")
                    + ' </span>\
                        <span style="display: inline-block;"> code : '
                    + magasin.replace("'", "''")
                    + " </span></span></br>"
                )
        ...
        if order_id:
            self._cr.execute(
                "update product_order set obs_devis='"
                + text2
                + "',obs_recap='"
                + text2
                + "',obs_atelier='"
            ...
            return {
                ...
                "views": [
                    (
                        self.env.ref(
                            "arfi.product_order_observation_create_view_form"
                        ).id,
                        "form",
                    )
                ],
    ...
    def action_echafaudage(self):
        ...
        return {
            ...
            "views": [
                (
                    self.env.ref("arfi.product_kks_echafaudage_create_view_form").id,
                    "form",
                )
            ],
            "target": "new",
    ...
    def action_pdr(self):
        ...
        return {
            ...
            "res_id": self.id,
            "views": [
                (self.env.ref("arfi.product_kks_pdr_create_view_form").id, "form")
            ],
            "target": "self",
            "flags": {"form": {"action_buttons": True}},
        }
...
class product_kks_piece(models.Model):
    ...
    piece_id = fields.Many2one(
        "product.piece",
        "Désignation",
        domain="[('appareil_id','=',appareil_id),('appareil_id','!=',None')]",
        change_default=True,
    )
    no_piece = fields.Char(
        related="piece_id.no_piece", string="REP", store=True, readonly=True
    )
    ...
    def open_piece(self):
        if self.piece_id:
            return {
                ...
                "res_id": self.magasin_id.id,
                "views": [
                    (self.env.ref("arfi.product_magasin_photo_form_view").id, "form")
                ],
                "target": "new",
                "flags": {"form": {"action_buttons": True}},
            }
...
class product_kks_stock(models.Model):
    ...
    sequence = fields.Integer(
        related="info_id.sequence", string="Sequence", store=True, readonly=True
    )
...
class product_magasin(models.Model):
    ...
    def action_select_info(self):
        ...
        for obj in self.stock_ids:
            self._cr.execute(
                "update product_info set variant='f' where id=" + str(obj.info_id.id)
            )
            self._cr.commit()
        return {
            "type": "ir.actions.act_window",
            ...
            "view_type": "form",
            "views": [
                (self.env.ref("arfi.view_product_magasin_stock_wizard_form").id, "form")
            ],
    ...
    supplier_ids = fields.One2many(
        "product.kks.supplier", "magasin_id", string="Fournisseurs", readonly=False
    )
    stock_ids = fields.One2many(
        "product.kks.stock", "magasin_id", string="Stock", readonly=False
    )
    ...
    unite_magasin = fields.Char(
        compute="_get_stock", string="Unité", readonly=True, store=True
    )
...
class product_kks_facture(models.Model):
    ...
    type_facture = fields.Selection(
        [("DEM", "DEM"), ("SMP", "SMP")], "Type facture", default="DEM"
    )
    ...
    facture_line = fields.One2many(
        "product.kks.facture.line", "facture_id", "Lignes", readonly=True
    )
    montant_ht = fields.Float(
        compute="_amount_all", string="Montant HT", readonly=True, store=True
    )
    tva = fields.Float(
        compute="_amount_all", string="TVA (20%)", readonly=True, store=True
    )
    montant_ttc = fields.Float(
        compute="_amount_all", string="Montant TTC", readonly=True, store=True
    )
    ...
    def _print_facture(self, data):
        data["form"].update(self.read(["id"])[0])
        return self.env["report"].get_action(
            self, "arfi.action_report_productkksfacturechangement", data=data
        )
    ...
    def _print_pv(self, data):
        data["form"].update(self.read(["id"])[0])
        return self.env["report"].get_action(
            self, "arfi.action_report_productkkspvchangement", data=data
        )
    ...
    def _print_pv_excel(self, data):
        ...
        titre = (
            "Désignation "
            + record.type_extraction
            + " arrêt unité "
            + record.unite_id.code
            + " du "
            + record.arret_id.date
        )
    ...
    def _print_facture_excel(self, data):
        ...
        style_nombre = workbook.add_format(
            {
                "num_format": "#,##0.00",
                "text_wrap": True,
                "bold": 1,
...
class product_kks_pps(models.Model):
    ...
    type_file = fields.Selection(
        [("prev", "Plan Prévention")], "Type du fichier", default="prev", required=False
    )
    ...
    def download_file(self):
        ...
        for res in self.env.cr.fetchall():
            ...
            data = res[3]
        filename = (
            filename.split(".")[0] + "_" + time.strftime("%H%M%S") + file_extension
        )
                        ...
                        for paragraph in cell.paragraphs:
                            ...
                            if "Client" in paragraph.text:
                                ...
                                run = paragraph.add_run(
                                    "Client : " + self.kks_id.customer_id.name
                                )
                                run.bold = True
                                paragraph.style = document.styles["Normal"]
