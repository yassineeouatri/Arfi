...
class res_partner(models.Model):
    ...
    order_ids = fields.One2many('product.order','customer_id','Commandes',readonly=True)
    ...
    def generer_contrat_report_excel(self,data):
        ...
        style_number = workbook.add_format({'num_format' : '#,##0.00', 'text_wrap' : True,'bold' :1,  'align' : 'center','valign' : 'vcenter','top' : 1, 'bottom' : 1, })
        ...
        records = self.env['product.kks.tarif'].search([('customer_id', '=', self.id)], order="kks_id asc")
        x=x+1
        for record in records:   
            magasin =''     
            pieces = self.env['product.piece'].search([('name','=',record.appareil_id.name)], limit=1)
            if pieces:
                magasins = self.env['product.kks.piece'].search([('piece_id', '=', pieces.id)], limit=1)
                if magasins : 
...
class res_supplier(models.Model):
    ...
    def get_code(self):
        i = '1'
        cr = self.env.cr
        cr.execute("""select cast(max(cast(code as numeric))+1 as text) from res_supplier""")
    ...
    purchase_ids = fields.One2many('product.purchase','supplier_id','Liste des achats',readonly=True)
