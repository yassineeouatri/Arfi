...
class product_invoice(models.Model):
    ...
    def _amount_all(self):
        ...
        for obj in self:
            ...
            montant_ht = montant * (
                ...
                - (
                    (
                        obj.retenue_pourcentage
                        + obj.tva_transport_pourcentage
            ...
            obj.update(
                {
                    ...
                    "tva_transport": (
                        montant * (((obj.tva_transport_pourcentage) * 1.00 / 100))
                    ),
    ...
    def compute_date_echeance(self):
        ...
        for obj in self:
            if obj.date_invoice:
                date_echeance = (
                    datetime.datetime.strptime(obj.date_invoice, "%Y-%m-%d")
    ...
    def get_name(self):
        ...
        for res in cr.fetchall():
            if res[0]:
                i = res[0]
        str(i.zfill(6)) + "/" + str(time.strftime("%Y"))[2:4]

        return "/"

    name = fields.Char("Facture N°", default=get_name, size=9, copy=False)
    type = fields.Selection(
        [("invoice", "Facture"), ("devis", "Devis"), ("avoir", "Avoir")],
        "Type",
        copy=True,
    )
    ...
    date_echeance = fields.Date(
        compute="compute_date_echeance", string="Date Facture", copy=False, store=True
    )
    ...
    montant_text = fields.Text(
        compute="_amount_all", string="Montant", readonly=True, store=True
    )
    invoice_line_ids = fields.One2many(
        "product.invoice.line", "invoice_id", "Lignes", copy=True
    )
    remise_pourcentage = fields.Float("Remise (%)", copy=True)
    remise = fields.Float(
        compute="_amount_all", string="Remise", readonly=True, store=True
    )
    tva_transport_pourcentage = fields.Float("TVA Transport (%)", copy=True)
    tva_transport = fields.Float(
        compute="_amount_all", string="TVA Transport", readonly=True, store=True
    )
    retenue_pourcentage = fields.Float("Retenue Garantie (%)", copy=True)
    retenue = fields.Float(
        compute="_amount_all", string="Retenue", readonly=True, store=True
    )
    tva = fields.Float(
    ...
    date_paiement = fields.Date(
        related="recovery_id.date_paiement", string="Date Paiement", copy=False
    )
    mode_paiement = fields.Selection(
        related="recovery_id.mode_paiement", string="Mode Paiement", copy=False
    )
    montant_paiement = fields.Float(
        related="recovery_id.montant_paiement", string="Montant Paiement", copy=False
    )
        ...
        for record in self:
            if record.name:
                if len(record.name) != 9:
                    raise ValidationError(
                        _(
                            "Attention! Le numéro de la facture doit contenir 9 caractères."
                        )
                    )
                if (
                    record.name[6:7] != "/"
                    or not record.name[0:6].isdigit()
                    or not record.name[7:9].isdigit()
                ):
                    raise ValidationError(
                        _(
                            "Attention! Le numéro de la facture doit être de la forme 999999/99."
                        )
                    )
    ...
    def _invoice_print(self, data):
        ...
        return self.env["report"].get_action(
            self, "arfi.action_report_productinvoice", data=data
        )
...
class product_invoice_arret(models.Model):
    ...
    invoice_line_ids = fields.One2many(
        "product.invoice.arret.line", "invoice_id", "Lignes", copy=True
    )
    tva = fields.Float(
        compute="_amount_all", string="TVA (20%)", readonly=True, store=True
    )
...
class product_devis(models.Model):
    ...
    devis_line_ids = fields.One2many(
        "product.devis.line", "devis_id", "Lignes", copy=True
    )
    remise_pourcentage = fields.Float("Remise (%)", copy=True)
    remise = fields.Float(
        compute="_amount_all", string="Remise", readonly=True, store=True
    ...
    def action_convert_to_invoice(self, default=None):
        ...
        if default is None:
            default = {}
        invoice_id = self.env["product.invoice"].create(
            {
                "customer_id": self.customer_id.id,
                "company_id": self.company_id.id,
                "city": self.city,
    ...
    def _devis_print(self, data):
        data["form"].update(self.read(["id"])[0])
        return self.env["report"].get_action(
            self, "arfi.action_report_productdevis", data=data
        )
...
class res_company(models.Model):
    ...
    def _compute_rib(self):
        ...
        for obj in self:
            obj.update(
                {
                    "rib": str(obj.bank_id.code)
                    + " "
                    + str(obj.city_id.code)
                    + " "
                    + str(obj.no_compte)
                    + " "
                    + str(obj.cle_rib),
                    "swift": obj.bank_id.code_bank
                    + obj.bank_id.code_country
                    + obj.bank_id.code_emplacement
                    + obj.bank_id.code_branche,
                }
            )
    ...
    swift = fields.Char(
        compute="_compute_rib", string="SWIFT", readonly=True, store=True
    )
...
class product_invoice_recovery(models.Model):
    ...
    invoice_ids = fields.One2many(
        "product.invoice", "recovery_id", "Factures", readonly=True
    )
    date_invoice = fields.Date(
        related="invoice_id.date_invoice", string="Date Facture", copy=False
    )
    date_paiement = fields.Date("Date Paiement", copy=False)
    montant_paiement = fields.Float("Montant Payé")
    mode_paiement = fields.Selection(
        [("Cheque", "Chèque"), ("Virement", "Virement"), ("Espece", "Espèce")],
        "Mode Paiement",
    )
    no_cheque = fields.Char("N° Chèque")
    no_virement = fields.Char("N° Virement")
    montant_ttc = fields.Float(
        related="invoice_id.montant_ttc",
    ...
    def action_add_invoice(self):
        if self.invoice_id:
            self._cr.execute(
                "update product_invoice set recovery_id="
                + str(self.id)
                + "\
                                where id="
                + str(self.invoice_id.id)
                + ""
            )
            self._cr.commit()
