...
class product_datetarif(models.Model):
    ...
    def compute_name(self):
        for obj in self:
            if obj.date:
                obj.name = datetime.datetime.strptime(obj.date, "%Y-%m-%d").strftime(
                    "%d/%m/%Y"
                )
            else:
                obj.name = ""
...
class product_operation(models.Model):
    ...
    application = fields.Selection(
        [("appareil", "Appareil"), ("piece", "Pièce")], "S'applique à", required=False
    )
...
class product_essai_organisme(models.Model):
    ...
    type = fields.Selection(
        [("person", "Personne"), ("organisme", "Organisme")], "Type"
    )
    name = fields.Char("Nom", required=False)
...
class product_product_material_reference(models.Model):
    ...
    material_search_id = fields.Many2one(
        "product.product.material.search", string="N° Arfi"
    )
...
class product_product_material(models.Model):
    ...
    reference_ids = fields.One2many(
        "product.product.material.reference",
        "material_id",
        string="Références",
        readonly=False,
    )
    ...
    def name_get(self):
        ...
        resuhh = []
        for record in self:
            resuhh.append((record.id, record.name))
...
class product_product_material_search(models.Model):
    ...
    def onchange_material_id(self):
        ...
        if self.material_id:
            ...
            self._cr.execute(
                "update product_product_material_search set designation='"
                + str(designation)
                + "'"
            )
            self._cr.commit()
            ...
            self._cr.commit()
        reference_ids = self.env["product.product.material.reference"].search(
    ...
    def onchange_norme_id(self):
        if self.norme_id:
            if self.material_id.id:
                ...
                return {
                    "value": {"reference_id": None},
                    "domain": {"reference_id": [("norme_id", "=", self.norme_id.id)]},
                }
    ...
    def case_validate(self):
        if self.piece_id and self.material_id:
            self._cr.execute(
                "update product_piece set material_id="
                + str(self.material_id.id)
                + "\
                            where id="
