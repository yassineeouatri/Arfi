...
class product_template(models.Model):
    ...
    def _compute_piece_count(self):
        read_group_res = self.env['product.piece'].read_group([('appareil_id', 'in', self.ids)], ['appareil_id'], ['appareil_id'])
    ...
    def _compute_procedure_count(self):
        read_group_res = self.env['product.procedure'].read_group([('appareil_id', 'in', self.ids)], ['appareil_id'], ['appareil_id'])
        group_data = dict((data['appareil_id'][0], data['appareil_id_count']) for data in read_group_res)
        for obj in self:
            obj.procedure_count = group_data.get(obj.id, 0)  
    def _compute_outillage_count(self):
        read_group_res = self.env['product.appareil.outillage'].read_group([('appareil_id', 'in', self.ids)], ['appareil_id'], ['appareil_id'])
        group_data = dict((data['appareil_id'][0], data['appareil_id_count']) for data in read_group_res)
        for obj in self:
    ...
    maker_id = fields.Many2one('product.template.maker','Fabriquant', required=False, change_default=True ,help="")
    ...
    parent_id = fields.Many2one('product.template','Appareil', select=True, ondelete='cascade')
    material_id = fields.Many2one('product.product.material','Matière')
    piece_ids = fields.One2many('product.piece', 'appareil_id', string='Pièces', copy=True)
    procedure_ids = fields.One2many('product.procedure', 'appareil_id', string='Procédures')
    instruction_ids = fields.One2many('product.procedure', 'appareil_id', string='Instructions de Maintenance' ,
                                      domain=[('type_file','=','ins')])
    maitenance_ids = fields.One2many('product.procedure', 'appareil_id', string='Manuel de Maintenance' ,
    ...
    def action_select_attribute(self):
        ...
        for obj in self.attribute_line_ids:
            self._cr.execute("update product_attribute set create_variant='f' where id="+str(obj.attribute_id.id))
            self._cr.commit()
    ...
    def action_select_operation(self):
        ...
        for obj in self.operation_ids:
            self._cr.execute("update product_operation set variant='f' where id="+str(obj.operation_id.id))
            self._cr.commit()
        res=self.env['ir.actions.act_window'].for_xml_id('arfi','open_product_appareil_order_operation_wizard')
    ...
    def action_select_nature(self):
        ...
        for obj in self.price_ids:
            ...
            self._cr.commit()
        res = self.env['ir.actions.act_window'].for_xml_id('arfi', 'open_product_template_nature')
        res['context'] = {'default_appareil_id': self.id}
    ...
    def action_select_procedure_it(self):
        ...
        return  {
                ...
                'view_mode': 'form',
                'view_type': 'form',
                'views': [(self.env.ref('arfi.view_product_template_procedure_it_form').id, 'form')],
                'context' : {'default_appareil_id' : self.id,
                         'default_directory_id' : self.return_directory_id("Instructions de Travail"),
    ...
    def action_select_procedure_mm(self):
        ...
        return  {
                ...
                'view_mode': 'form',
                'view_type': 'form',
                'views': [(self.env.ref('arfi.view_product_template_procedure_mm_form').id, 'form')],
                'context' : {'default_appareil_id' : self.id,
                         'default_directory_id' : self.return_directory_id("Manuel de Maintenance"),
    ...
    def action_select_procedure_ope(self):
        ...
        return  {
                ...
                'view_mode': 'form',
                'view_type': 'form',
                'views': [(self.env.ref('arfi.view_product_template_procedure_ope_form').id, 'form')],
                'context' : {'default_appareil_id' : self.id,
    ...
    def action_edit(self):
        return {
            ...
            'view_type': 'form',
            'res_id': self.id,
            'views': [(self.env.ref('arfi.product_template_appareil_form_view').id, 'form')],
    ...
    def action_procedure(self):
        return {
            ...
            'view_type': 'form',
            'res_id': self.id,
            'views': [(self.env.ref('arfi.product_template_appareil_procedure_create_form_view').id, 'form')],
    ...
    def action_outillage(self):
        ...
        return {
            ...
            'view_type': 'form',
            'res_id': self.id,
            'views': [(self.env.ref('arfi.product_template_appareil_outillage_create_form_view').id, 'form')],
    ...
    def _print_product_template_cartouche(self, data):
        ...
        return self.env['report'].get_action(self, 'arfi.action_report_producttemplatecartouche', data=data)
    @api.multi
    ...
    def _print_product_template_outillage(self, data):
        data['form'].update(self.read(['id'])[0])
        return self.env['report'].get_action(self, 'arfi.action_report_producttemplateoutillage', data=data)
    @api.multi
    ...
    def _print_product_template_outillage_tarage(self, data):
        data['form'].update(self.read(['id'])[0])
        return self.env['report'].get_action(self, 'arfi.action_report_producttemplateoutillagetarage', data=data)
...
class product_piece(models.Model):
    ...
    def _compute_procedure_count(self):
        read_group_res = self.env['product.procedure'].read_group([('piece_id', 'in', self.ids)], ['piece_id'], ['piece_id'])
        group_data = dict((data['piece_id'][0], data['piece_id_count']) for data in read_group_res)
        for obj in self:
            obj.procedure_count = group_data.get(obj.id, 0)  
    def compute_order(self):
        ...
        for obj in self:
            if obj.no_piece:
                ...
                if no_piece.isdigit():
                    rslt = no_piece  
                elif no_piece.count('.')==1 and ((partition[0].isdigit() and partition[1]=='.' and partition[2].isdigit())
                            or (partition[0]=='' and partition[1]=='.' and partition[2].isdigit())
                            or (partition[0].isdigit() and partition[1]=='.' and partition[2]=='')):
                        rslt = float(no_piece)
                elif no_piece.count('.')==1 and ((partition[0].isdigit() and partition[1]=='.' and not partition[2].isdigit())):
                            rslt = float(partition[0])
                elif no_piece.count('.')==2 and partition[0].isdigit() and partition[1]=='.' and partition[2].isdigit():
                    rslt= float(str(partition[0])+'.'+str(partition[2]))
                elif no_piece.count('.')==2 and partition[0].isdigit() and partition[1]=='.' and not partition[2].isdigit():
                    rslt= float(str(partition[0]))
                elif no_piece.count('.')==0 and not no_piece.isdigit() and len(no_piece)==2 and no_piece[:1].isdigit() and not no_piece[-1:].isdigit():
                    rslt=float(no_piece[:1])
                elif len(no_piece)==3 and no_piece[:2].isdigit() and not no_piece[-1:].isdigit():
    ...
    def update_order(self):
        ...
        for res in self.env.cr.fetchall():
            ...
            self._cr.execute("update product_piece set order_="+str(rslt)+" where id="+str(res[0]))
            self._cr.commit()
        return True
    ...
    poids = fields.Char('Poids')
    order_id = fields.Many2one('product.order','Commande')
    appareil_id = fields.Many2one('product.template','Appareil', select=True, ondelete='cascade')
    material_id = fields.Many2one('product.product.material','Matière')
    operation_ids = fields.One2many('product.order.operation', 'piece_id', string='Opérations' ,domain=[('order_','=',1)])
    procedure_ids = fields.One2many('product.procedure', 'piece_id', string='Procédures')
    instruction_ids = fields.One2many('product.procedure', 'piece_id', string='Instructions de Maintenance' ,
                                      domain=[('type_file','=','ins')])
    maitenance_ids = fields.One2many('product.procedure', 'piece_id', string='Manuel de Maintenance' ,
                                      domain=[('type_file','=','man')])
    operatoire_ids = fields.One2many('product.procedure', 'piece_id', string='Mode Opératoire' ,
                                      domain=[('type_file','=','ope')])
    param1 = fields.Char('Param1')
    download_image = fields.Boolean('Image téléchargée',default=False)
    order_ = fields.Float(compute="compute_order",String="Name", readonly=False,store=True,) 
    procedure_count = fields.Integer(compute="_compute_procedure_count",type='integer', string="# of Procedures",method=True) 
    ...
    def action_delete(self):
        ...
        res=self.env['ir.actions.act_window'].for_xml_id('arfi','open_product_piece_delete_wizard')
        res['context'] ={'default_piece_id' : self.id}       
        
        return  res
        
    def action_image(self):
        self._cr.execute("select id from ir_attachment where res_id="+str(self.id)+" and res_model='product.piece' and res_field='image'")
        for res in self.env.cr.fetchall():
            file_id=res[0]
            if file_id:
                url="/web/content/"+str(file_id)
    ...
    def action_select_operation(self):
        ...
        for obj in self.operation_ids:
            self._cr.execute("update product_operation set variant='f' where id="+str(obj.operation_id.id))
            self._cr.commit()
        res=self.env['ir.actions.act_window'].for_xml_id('arfi','open_product_piece_order_operation_wizard')
        res['context'] ={'default_piece_id' : self.id,'default_appareil_id' : self.appareil_id.id,'default_order_id' : self.order_id.id}
    ...
    def action_material(self):
        ...
        for res in self.env.cr.fetchall():
            obj_id=res[0]
        self._cr.execute("update product_product_material_search set material_id=Null,designation='',norme_id=Null,reference_id=Null,\
                        piece_id="+str(self.id))
        self._cr.commit()
        ...
        if self.material_id:
            ...
            self._cr.execute("update product_product_material_search set material_id="+str(self.material_id.id)+",designation='"+designation+"'")
            self._cr.commit()
        ...
        return {
            ...
            'res_id': obj_id,
            'target': 'new',
            'views': [(self.env.ref('arfi.product_product_material_search_form_view').id, 'form')],
            }
    def action_material_readonly(self):
        ...
        return {
            ...
            'res_id': obj_id,
            'target': 'new',
            'views': [(self.env.ref('arfi.product_product_material_search_readonly_form_view').id, 'form')],
            }
    ...
    def action_procedure(self):
        ...
        return {
            ...
            'view_type': 'form',
            'res_id': self.id,
            'views': [(self.env.ref('arfi.product_piece_procedure_create_form_view').id, 'form')],
            'target': 'new',
            }
...
class product_category(models.Model):
    ...
    type = fields.Selection([('type','Type'), ('ss_type','Sous Type')], 'Type', default='type', help="")
    description = fields.Char("Déscription")
    ss_categ_attribute_ids = fields.Many2many('product.attribute','ss_categ_attribute_rel','ss_categ_id','attribute_id','Informations Techniques')
    complete_name = fields.Char(compute="complete_name_get",String="Name", readonly=True,store=True,) 
...
class product_appareil_outillage(models.Model):
    ...
    code = fields.Char(related='outillage_id.code',string="Code", store=True ,readonly=True)
...
class product_appareil_price(models.Model):
    ...
    def _onchange_sale_price(self):
        if self.appareil_id:
            self._cr.execute(""" UPDATE product_kks_tarif a SET montant={0} 
                              FROM product_template b WHERE a.appareil_id=b.id
                              AND  b.name='{1}' AND nature_id={2}""".format(self.sale_price, self.appareil_id.name, self.nature_id.id))

    @api.onchange('purchase_price')  # if these fields are changed, call method
    def _onchange_purchase_price(self):
        if self.appareil_id:
            self._cr.execute(""" UPDATE product_kks_tarif a SET montant2={0} 
                                          FROM product_template b WHERE a.appareil_id=b.id
                                          AND  b.name='{1}' AND nature_id={2}""".format(self.purchase_price,
                                                                                        self.appareil_id.name,
                                                                                        self.nature_id.id))
