...
class product_purchase(models.Model):
    ...
    def action_select_evaluation(self):
        ...
        for obj in self.purchase_evaluation_ids:
            self._cr.execute("update product_purchase_type_evaluation set variant='f' where id="+str(obj.type_evaluation_id.id))
            self._cr.commit()
        return  {
                ...
                'view_mode': 'form',
                'view_type': 'form',
                'views': [(self.env.ref('arfi.view_product_purchase_evaluation_wizard_form').id, 'form')],
    ...
    note_evaluation = fields.Integer(compute='_amount_evaluation',string='Note évaluation', readonly=True, store=True)
    purchase_line_ids = fields.One2many('product.purchase.line','purchase_id','Lignes',copy= True)
    purchase_evaluation_ids = fields.One2many('product.purchase.evaluation','purchase_id','Lignes',copy= True)
...
class product_purchase_line(models.Model):
    ...
    def _onchange_magasin(self):
        rslt=''
        pieces=self.env['product.kks.piece'].search([('magasin_id', '=', self.magasin_id.id)]) 
        for piece in pieces:
...
class product_purchase_activity(models.Model):
    ...
    def get_code(self):
        i = '1'
        cr = self.env.cr
        cr.execute("""select cast(max(cast(code as numeric))+1  as text)
...
class product_bc(models.Model):
    ...
    def print_bc_report_excel(self):
        ...
        style_title = workbook.add_format({'bg_color': '#003366', 'color': 'white', 'text_wrap': True, 'bold': 1, 'align': 'center','valign': 'vcenter','top': 1, 'bottom': 1, })
        style_titre = workbook.add_format({'color': '#003366', 'text_wrap': True, 'bold': 1, 'align': 'center', 'valign': 'vcenter', 'font_size': 18, 'font_name': 'tahoma'})
        style_titre2 = workbook.add_format({'text_wrap': True, 'bold': 1, 'align': 'center', 'valign': 'vcenter','font_size': 14, 'font_name': 'tahoma', 'border': 2})
        style = workbook.add_format({'text_wrap': True, 'bold': 1, 'align': 'center', 'valign': 'vcenter', 'top': 1, 'bottom': 1, })
        style_number = workbook.add_format({'num_format': '#,##0.00','text_wrap': True, 'bold': 1, 'align': 'right', 'valign': 'vcenter', 'top': 1, 'bottom': 1, })
        style__ = workbook.add_format({'text_wrap': True, 'bold': 1, 'right': 1, })
        style_ = workbook.add_format({'text_wrap': True, 'bold': 1, 'left': 1, })
        ...
        titre = 'Désignation :'+self.travaux_id.name+' arrêt '+self.arret_id.name+' unité '+self.unite_id.name
...
class product_bc_line(models.Model):
    ...
    maker_id = fields.Many2one('product.template.maker', 'Marque',related='kks_id.maker_id',store=True, readonly=False)
    ...
    def _onchange_nature_id(self):
        if self.nature_id and self.appareil_id :
            ...
            objs = self.env['product.appareil.price'].search([('appareil_id', '=', self.appareil_id.id),
...
class product_bc_piece(models.Model):
    ...
    supplier_id = fields.Many2one('res.supplier', 'Fournisseur', copy=True)
    bc_line_ids = fields.One2many('product.bc.piece.line', 'bc_id', 'Lignes de BC', copy=True)
