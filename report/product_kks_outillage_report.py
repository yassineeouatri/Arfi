...
class ReportKKSOUTILLAGE(models.AbstractModel):
    ...
    def render_html(self, docids, data=None):
        ...
        records = self.env['product.kks.outillage.arret'].search([('customer_id', '=', docs.customer_id.id),
        ...
        return self.env['report'].render('arfi.action_report_productkksoutillage', docargs)
...
class ReportKKSOUTILLAGETARAGE(models.AbstractModel):
    ...
    def render_html(self, docids, data=None):
        self.model = self.env.context.get('active_model')
        docs = self.env[self.model].browse(self.env.context.get('active_id'))
        records = self.env['product.kks.outillage.tarage.arret'].search([('customer_id', '=', docs.customer_id.id),
                                                        ('arret_id', '=', docs.arret_id.id)])
        ...
        return self.env['report'].render('arfi.action_report_productkksoutillagetarage', docargs)