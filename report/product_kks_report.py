...
class product_kks_piece_report(models.Model):
    ...
    piece_id = fields.Many2one("product.piece", "Pièce", required=True)
    magasin_id = fields.Many2one("product.magasin", "Code Magasin", readonly=True)
    ...
    def init(self):
        ...
        self._cr.execute(
           ...
           CREATE or REPLACE view product_kks_piece_report as (
                SELECT row_number() OVER () as id,t.*
                FROM (SELECT magasin.id as magasin_id,piece.piece_id,
                CASE WHEN piece.designation IS NULL THEN '' ELSE piece.designation END||
                CASE WHEN magasin.designation IS NULL THEN '' ELSE  magasin.designation END as designation,
			magasin.photo_name,magasin.photo,magasin.stock_magasin, magasin.unite_magasin
                    FROM product_magasin magasin
                    LEFT JOIN 
		    (SELECT pkp.piece_id,pkp.magasin_id,pp.name as designation FROM
                    product_kks_piece pkp
                    INNER JOIN product_piece pp on pp.id=pkp.piece_id) as piece ON piece.magasin_id=magasin.id
