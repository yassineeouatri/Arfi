...
class ReportKKSEchafaudage(models.AbstractModel):
    ...
    def render_html(self, docids, data=None):
        ...
        return self.env['report'].render('arfi.action_report_productkksechafaudage', docargs)
...
class ReportKKSEchafaudageArret(models.AbstractModel):
    ...
    def render_html(self, docids, data=None):
        ...
        self.model = self.env.context.get('active_model')
        docs = self.env[self.model].browse(self.env.context.get('active_id'))
        records = self.env['product.kks.echafaudage.arret'].search([('customer_id', '=', docs.customer_id.id),
        ...
        return self.env['report'].render('arfi.action_report_productkksechafaudagearret', docargs)
class ReportKKSFactureEchafaudageArret(models.AbstractModel):
    ...
    def render_html(self, docids, data=None):
        ...
        self.model = self.env.context.get('active_model')
        docs = self.env[self.model].browse(self.env.context.get('active_id'))
        records = self.env['product.kks.facture.echafaudage.arret'].search([('customer_id', '=', docs.customer_id.id),
                                                                            ('arret_id', '=', docs.arret_id.id)])
        unite=''
        if records:
        ...
        return self.env['report'].render('arfi.action_report_productkksfactureechafaudagearret', docargs)